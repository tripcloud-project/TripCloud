...
public class ControllerExceptionHandler {
    ...
    private ResponseEntity<?> createError(ErrorCode code){
    ...
    public ResponseEntity<?> notFoundUserException() {
    	return createError(ErrorCode.NOT_FOUND_USER);
    }

    // 키가 중복되는 멤버 삽입 시도
    @ExceptionHandler(value = {DuplicateMemberException.class})
    public ResponseEntity<?> duplicateUserException() {
    	return createError(ErrorCode.DUPLICATE_MEMBER);
    }
    ...
    public ResponseEntity<?> handleUploadFailException(UploadFailException e) {
    	return createError(ErrorCode.UPLOAD_FAIL);
    }
